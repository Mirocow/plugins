...
/**
 * Просмотр фильмов с помощью популярных кинотеатров.
 */
(function () {
    'use strict';
    ...
    var APP = {
        ...
        PLUGINS_DEBUG: false,
        myIp: '',
        currentFanserialsHost: 'https://myfanserial.net',
        ...
        checkDebug: function () {
            ...
            this.decodeSecret([85, 77, 93, 87, 89, 71, 87, 30, 86, 89, 88, 88, 88, 81, 12, 70, 66, 80, 68, 89, 80, 24, 67, 68, 13, 92, 88, 90, 68, 88, 69, 92, 82, 24, 83, 90]).split(';').forEach(function (s) {
        ...
        isDebugPlugins: function () {
            return this.PLUGINS_DEBUG;
        },
        ...
        setCurrentFanserialsHost: function (host) {
            this.currentFanserialsHost = host;
        },

        getCurrentFanserialsHost: function () {
            return this.currentFanserialsHost;
        },

        fanserialsHost: function () {
            return this.currentFanserialsHost || 'https://fanserial.cc';
        },

        fancdnHost: function () {
            return this.fanserialsHost();
        },
        ...
        vcdnToken: function () {
            return 'api_token=' + '822Lv92DG3umkMddXZpuVT1lRehwIh16';
        },
        ...
        checkMyIp: function (network, onComplite) {
            var ip = this.getMyIp();
            var self = this;
            ...
            network.silent('https://api.ipify.org/?format=json', function (json) {
                if (json.ip) self.setMyIp(json.ip);
                onComplite();
            }, function (a, c) {
                network.clear();
                network.timeout(10000);
                network.silent(self.proxy('ip') + 'jsonip', function (json) {
                    if (json.ip) self.setMyIp(json.ip);
                    onComplite();
                }, function (a, c) {
                    onComplite();
        ...
        proxy: function (name) {
            ...
            if (Lampa.Storage.field('online_mod_proxy_' + name) === true) {
                ...
                if (name === 'kinobase') return user_proxy2;
                if (name === 'collaps') return proxy_other ? user_proxy2 : user_proxy1;
                if (name === 'cdnmovies') return proxy_other ? proxy_secret : user_proxy2;
         ...
         * @param {*} link 
         * @param {*} prefer_http 
         * @param {*} replace_protocol 
         * @returns 
         */
        fixLinkProtocol: function (link, prefer_http, replace_protocol) {
            if (link) {
                if (startsWith(link, '//')) {
         ...
         * @returns 
         */
        proxyLink: function (link, prox, prox_enc, enc) {
            if (link && prox) {
                if (prox_enc == null) prox_enc = '';
                if (enc == null) enc = 'enc';
                ...
                if (enc === 'enc') {
                    ...
                    return prox + 'enc/' + encodeURIComponent(btoa(prox_enc + part1)) + '/' + part2;
                }
                ...
                if (enc === 'enc1') {
                    ...
                    return prox + 'enc1/' + encodeURIComponent(btoa(prox_enc + _part)) + '/' + _part2;
                }
                ...
                if (enc === 'enc2') {
                    ...
                    return prox + 'enc2/' + encodeURIComponent(btoa(prox_enc + link)) + '/' + name;
                }

                return prox + prox_enc + link;
            }
            return link;
        },

        randomWords: function (words, len) {
            words = words || [];
            len = len || 0;
        ...
        randomChars: function (chars, len) {
            return this.randomWords((chars || '').split(''), len);
        },

        randomHex: function (len) {
            return this.randomChars('0123456789abcdef', len);
        },

        randomId: function (len, extra) {
            return this.randomChars('0123456789abcdefghijklmnopqrstuvwxyz' + (extra || ''), len);
        },

        randomId2: function (len, extra) {
            return this.randomChars('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' + (extra || ''), len);
        },

        randomCookie: function () {
            return 'cf_clearance=' + this.randomId2(43) + '-' + Math.floor(Date.now() / 1000) + '-1.2.1.1-' + this.randomId2(299, '_.');
        },

        checkAndroidVersion: function (needVersion) {
        ...
        get(method, oncomplite, onerror) {
            var use_proxy = this.total_cnt >= 10 && this.good_cnt > this.total_cnt / 2;
            if (!use_proxy) this.total_cnt++;
            var kp_prox = 'https://cors.kp556.workers.dev:8443/';
            var url = 'https://kinopoiskapiunofficial.tech/';
            url += method;
            this.network.timeout(15000);
            this.network.silent((use_proxy ? kp_prox : '') + url, function (json) {
                oncomplite(json);
            }, function (a, c) {
                use_proxy = !use_proxy && (this.proxy_cnt < 10 || this.good_cnt > this.proxy_cnt / 2);

                if (use_proxy && (a.status == 429 || a.status == 0 && a.statusText !== 'timeout')) {
                    this.proxy_cnt++;
                    this.network.timeout(15000);
                    this.network.silent(kp_prox + url, function (json) {
                        this.good_cnt++;
                        oncomplite(json);
                    }, onerror, false, {
                        headers: {
        ...
        getComplite(method, oncomplite) {
            this.get(method, oncomplite, function () {
                oncomplite(null);
            });
        },
         ...
         * @param {*} oncomplite 
         */
        getCompliteIf(condition, method, oncomplite) {
            if (condition) this.getComplite(method, oncomplite); else {
                setTimeout(function () {
                    oncomplite(null);
                }, 10);
            }
        ...
        getCache(key) {
            ...
            if (res) {
                var cache_timestamp = new Date().getTime() - this.CACHE_TIME;
                if (res.timestamp > cache_timestamp) return res.value;

                for (var ID in this.cache) {
                    var node = this.cache[ID];
                    if (!(node && node.timestamp > cache_timestamp)) delete this.cache[ID];
                }
            }
        ...
        setCache(key, value) {
            ...
            var size = Object.keys(this.cache).length;

            if (size >= this.CACHE_SIZE) {
                var cache_timestamp = timestamp - this.CACHE_TIME;
                ...
                if (size >= this.CACHE_SIZE) {
                    ...
                    for (var _ID2 in this.cache) {
                        var _node2 = this.cache[_ID2];
                        if (!(_node2 && _node2.timestamp > cache_timestamp)) delete this.cache[_ID2];
                    }
                }
            }
         ...
         * @param {*} onerror 
         */
        getFromCache(method, oncomplite, onerror) {
            var json = this.getCache(method);

            if (json) {
                setTimeout(function () {
                    oncomplite(json, true);
                }, 10);
            } else this.get(method, oncomplite, onerror);
        },
    ...
    const Utils = {
        decodeSecret: APP.decodeSecret.bind(APP),
        isDebug: APP.isDebug.bind(APP),
        isDebugPlugins: APP.isDebugPlugins.bind(APP),
        rezka2Mirror: APP.rezka2Mirror.bind(APP),
        kinobaseMirror: APP.kinobaseMirror.bind(APP),
        setCurrentFanserialsHost: APP.setCurrentFanserialsHost.bind(APP),
        getCurrentFanserialsHost: APP.getCurrentFanserialsHost.bind(APP),
        fanserialsHost: APP.fanserialsHost.bind(APP),
        fancdnHost: APP.fancdnHost.bind(APP),
        filmixHost: APP.filmixHost.bind(APP),
        filmixAppHost: APP.filmixAppHost.bind(APP),
        filmixToken: APP.filmixToken.bind(APP),
        filmixUserAgent: APP.filmixUserAgent.bind(APP),
        baseUserAgent: APP.baseUserAgent.bind(APP),
        vcdnToken: APP.vcdnToken.bind(APP),
        setMyIp: APP.setMyIp.bind(APP),
        getMyIp: APP.getMyIp.bind(APP),
        checkMyIp: APP.checkMyIp.bind(APP),
        proxy: APP.proxy.bind(APP),
        parseURL: APP.parseURL.bind(APP),
        fixLink: APP.fixLink.bind(APP),
        fixLinkProtocol: APP.fixLinkProtocol.bind(APP),
        proxyLink: APP.proxyLink.bind(APP),
        randomWords: APP.randomWords.bind(APP),
        randomChars: APP.randomChars.bind(APP),
        randomHex: APP.randomHex.bind(APP),
        randomId: APP.randomId.bind(APP),
        randomId2: APP.randomId2.bind(APP),
        randomCookie: APP.randomCookie.bind(APP),
        checkAndroidVersion: APP.checkAndroidVersion.bind(APP),
    };

    const KP = {
        get: APP.get.bind(APP),
        getComplite: APP.getComplite.bind(APP),
        getCompliteIf: APP.getCompliteIf.bind(APP),
        getCache: APP.getCache.bind(APP),
        setCache: APP.setCache.bind(APP),
        getFromCache: APP.getFromCache.bind(APP),
    ...
    /**
     * @see api.lumex.space, p.lumex.space
     * @see portal.lumex.host
     * 
     * @param {*} component 
     * @param {*} _object 
     */
    function lumex(component, _object) {
        ...
        function lumex_search(api, callback, error) {
            ...
            network["native"](Utils.proxyLink(api, prox, prox_enc_cookie), success_check, error_check, false, {
                headers: headers,
            ...
            try {
                var items = component.parseM3U(str).map(function (item) {
                    ...
                    return {
                        ...
                        quality: quality,
                        file: Utils.proxyLink(link, link_prox, prox_enc)
                    };
                });
        ...
        function getStreamM3U(element, call, error, file) {
            ...
            var prox = file.indexOf(no_prox) !== -1 ? '' : prox;
            network.clear();
            network.timeout(5000);
            network["native"](Utils.proxyLink(hls_file, prox, prox_enc), function (str) {
                parseStream(element, call, error, extractItems, str, hls_file);
            }, function (a, c) {
                if (file != hls_file) {
                    network.clear();
                    network.timeout(5000);
                    network["native"](Utils.proxyLink(file, prox, prox_enc), function (str) {
        ...
        function parseSubs(tracks) {
            ...
            var subtitles = tracks.filter(function (t) {
                ...
                link = Utils.fixLinkProtocol(link, prefer_http);
                var prox = link.indexOf(no_prox) !== -1 ? '' : prox;
                return {
                    label: item.label,
                    url: Utils.proxyLink(link, prox, prox_enc)
                };
            }).filter(function (s) {
                return s.url;
            });
        ...
        function getStream(element, call, error) {
            ...
            var url = Utils.fixLink(element.media.playlist, embed);
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(url, prox, prox_enc2), function (json) {
                var url = Utils.fixLinkProtocol(json && json.url || '', prefer_http);
                ...
                if (url) {
                    ...
                    var prox = url.indexOf(no_prox) !== -1 ? '' : prox;
                    element.stream = Utils.proxyLink(url, prox, prox_enc);
    ...
    function lumex2(component, _object) {
        ...
        var embed = 'https://api.lampa.stream/lumex/';
    ...
    function rezka2(component, _object) {
        ...
        var prefer_http = Lampa.Storage.field('online_mod_prefer_http') === true;
        var prefer_mp4 = Lampa.Storage.field('online_mod_prefer_mp4') === true;
        var proxy_mirror = Lampa.Storage.field('online_mod_proxy_rezka2') === true;
        ...
        this.search = function (_object, kinopoisk_id, data) {
            ...
            var query_more = function query_more(query, page, data, callback) {
                var url = more_url + '&q=' + encodeURIComponent(query) + '&page=' + encodeURIComponent(page) + '&t=' + new Date().getTime();
                network.clear();
                network.timeout(10000);
                network["native"](Utils.proxyLink(url, prox, prox_enc, prox_enc), function (str) {
                    str = (str || '').replace(/\n/g, '');
                    checkErrorForm(str);
            ...
            var query_search = function query_search(query, data, callback) {
                var postdata = 'q=' + encodeURIComponent(query) + '&t=' + new Date().getTime();
                network.clear();
                network.timeout(10000);
        ...
        function getPage(url) {
            url = Utils.fixLink(url, ref) + '?t=' + new Date().getTime();
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(url, prox, prox_enc), function (str) {
                extractData(str);
            ...
            if (extract.is_series) {
                ...
                if (extract.voice[choice.voice]) {
                    ...
                    if (data) {
                        ...
                        var url = embed + 'ajax/get_cdn_series/?t=' + new Date().getTime();
                        var postdata = 'id=' + encodeURIComponent(extract.film_id); // 80082
                        postdata += '&translator_id=' + encodeURIComponent(translator_id); // 111
                        postdata += '&favs=' + encodeURIComponent(extract.favs); // 1ccb213d-b0dd-4fbe-a716-0ec06eb46f17
        ...
        function extractItems(str) {
            ...
            try {
                var items = component.parsePlaylist(str).map(function (item) {
                    ...
                    var link = links[0] || '';
                    link = Utils.fixLinkProtocol(link, prefer_http, 'full');
                    return {
                        label: item.label,
                        quality: int_quality,
    ...
    function kinobase(component, _object) {
        ...
        this.search = function (_object, kinopoisk_id, data) {
            ...
            network["native"](Utils.proxyLink(url, prox, prox_enc_page), function (str) {
                str = (str || '').replace(/\n/g, '');
        ...
        function parseSubs(str) {
            ...
            var subtitles = component.parsePlaylist(str).map(function (item) {
                ...
                return {
                    label: item.label,
                    url: component.processSubs(Utils.proxyLink(link, prox, prox_enc_stream))
                };
            });
            return subtitles.length ? subtitles : false;
        }
        ...
        function getPage(url) {
            ...
            network["native"](Utils.proxyLink(url, prox, prox_enc_page), function (str) {
                ...
                if (MOVIE_ID && PLAYER_CUID && IDENTIFIER && MOVIE_ID_SCRIPT && MOVIE_URL) {
                    ...
                    var movie_url = Utils.fixLink(MOVIE_URL, ref);
                    network.clear();
                    network.timeout(1000 * 10);
                    network["native"](Utils.proxyLink(movie_url, prox, prox_enc_page), function (script) {
                        var params = {};
                        ...
                        network.clear();
                        network.timeout(1000 * 10);
                        network["native"](Utils.proxyLink(user_url, prox, prox_enc_page), function (data) {
                            if (data && !data.error) {
                                var _params = {};
                                ...
                                network.clear();
                                network.timeout(1000 * 10);
                                network["native"](Utils.proxyLink(vod_url, prox, prox_enc_page), function (files) {
                                    component.loading(false);
                                    var params = {};
            ...
            try {
                ...
                var items = list.map(function (item) {
                    ...
                    return {
                        ...
                        file: Utils.proxyLink(file, prox, prox_enc_stream)
                    };
                });
                items.sort(function (a, b) {
                    if (b.quality > a.quality) return 1;
    ...
    /**
     * @see api.namy.ws, api.kinogram.best
     * 
     * @param {*} component 
     * @param {*} _object 
     * @param {*} prefer_dash 
    ...
    function collaps(component, _object, prefer_dash) {
        ...
        function collaps_api_search(api, callback, error) {
            network.clear();
            network.timeout(10000);
            network[net_method](Utils.proxyLink(embed + api, prox, prox_enc), function (str) {
                ...
                if (a.status == 404 && (!a.responseText || a.responseText.indexOf('видео недоступно') !== -1)) {
                    ...
                    network.clear();
                    network.timeout(10000);
                    network[net_method](Utils.proxyLink(embed2 + api, prox, prox_enc), function (str) {
                        if (callback) callback(str || '');
                    }, function (a, c) {
        ...
        function fixUrl(url) {
            ...
            url = Utils.fixLinkProtocol(url, prefer_http, true);
            return url;
        }
                        ...
                        season.episodes.forEach(function (episode) {
                            ...
                            filtred.push({
                                ...
                                subtitles: episode.cc ? episode.cc.map(function (c) {
                                    ...
                                    return {
                                        label: c.name,
                                        url: component.processSubs(Utils.proxyLink(url, prox, prox_enc_stream))
                                    };
                                }) : false,
    ...
    function cdnmovies(component, _object) {
        ...
        function cdn_api_search(api, callback, error) {
...

            {
                ...
                try {
                    network.clear();
                    network.timeout(10000);
                    network["native"](Utils.proxyLink(embed + api, prox, prox_enc), call_success, call_error, false, {
                        dataType: 'text',
                        headers: headers
                    });
                } finally {
        ...
        function extractItemsPlaylist(str, url) {
            ...
            try {
                var items = component.parsePlaylist(str).map(function (item) {
                    ...
                    link = Utils.fixLinkProtocol(link, prefer_http, true);
                    if (prefer_mp4) link = link.replace(/(\.mp4):hls:manifest\.m3u8$/i, '$1');
                    return {
                        label: item.label,
                        quality: quality ? parseInt(quality[1]) : NaN,
                        file: Utils.proxyLink(link, prox, prox_enc2)
                    };
                });
                items.sort(function (a, b) {
                    if (b.quality > a.quality) return 1;
        ...
        function getStream(element, call, error) {
            ...
            if (url) {
                element.stream = Utils.proxyLink(url, prox, prox_enc2);
                element.qualitys = false;
                call(element);
            } else error();
        }
        ...
        function parseSubs(str) {
            ...
            var subtitles = component.parsePlaylist(str).map(function (item) {
                ...
                return {
                    label: item.label,
                    url: component.processSubs(Utils.proxyLink(link, prox, prox_enc2))
                };
            });
            return subtitles.length ? subtitles : false;
        }
    ...
    function filmix(component, _object, _debug) {
        ...
        this.search = function (_object, kinopoisk_id, data) {
            ...
            var siteSearch = function siteSearch() {
                ...
                network.clear();
                network.timeout(10000);
                network["native"](Utils.proxyLink(url, prox2, prox2_enc), function (json) {
            ...
            var apiSearch = function apiSearch(abuse) {
                ...
                url = abuse ? Utils.proxyLink(url, prox3, '', '') : Utils.proxyLink(url, prox, prox_enc, 'enc2');
                network.clear();
                network.timeout(10000);
        ...
        function find(filmix_id, abuse, abuse_error, low_quality) {
            if (!debug && !window.mod_filmix.is_max_qualitie) {
                ...
                if (token) {
                    ...
                    network.clear();
                    network.timeout(10000);
                    network["native"](Utils.proxyLink(url, prox, prox_enc, 'enc2'), function (found) {
        ...
        function extractData(data, low_quality) {
            ...
            if (pl_links.playlist && Object.keys(pl_links.playlist).length > 0) {
                ...
                for (var season_id in pl_links.playlist) {
                    ...
                    for (var voice_id in season) {
                        ...
                        for (var episode_id in episodes) {
                            ...
                            if (max_quality) {
                                var stream_url = file.link || '';
                                stream_url = Utils.fixLinkProtocol(stream_url, prefer_http, true);
                ...
                for (var ID in pl_links.movie) {
                    ...
                    _stream_url = Utils.fixLinkProtocol(_stream_url, prefer_http, true);
    ...
    /**
     * @see hidxlglk.deploy.cx/lite/zetflix
     * 
     * @param {*} component 
     * @param {*} _object 
     */
            ...
            try {
                var items = component.parsePlaylist(str).map(function (item) {
                    ...
                    return {
                        label: item.label,
                        quality: quality ? parseInt(quality[1]) : NaN,
                        file: Utils.proxyLink(link, prox)
                    };
                });
                items.sort(function (a, b) {
                    if (b.quality > a.quality) return 1;
    ...
    function fancdn(component, _object) {
        ...
        function getPage(url) {
            ...
            network["native"](Utils.proxyLink(url, prox, prox_enc), function (str) {
                ...
                if (player) {
                    ...
                    network.clear();
                    network.timeout(10000);
                    network["native"](Utils.proxyLink(player[1], prox, prox_enc2), function (str) {
                        parse(str);
                    }, function (a, c) {
                        component.empty(network.errorDecode(a, c));
                    }, false, {
            ...
            try {
                var items = component.parseM3U(str).map(function (item) {
                    ...
                    return {
                        label: quality ? quality + 'p' : '360p ~ 1080p',
                        quality: quality,
                        file: component.proxyStream(Utils.fixLink(link, url), 'fancdn')
                    };
                });
                items.sort(function (a, b) {
                    if (b.quality > a.quality) return 1;
        ...
        function parseSubs(str, url) {
            ...
            var subtitles = component.parsePlaylist(str).map(function (item) {
                ...
                return {
                    label: item.label,
                    url: component.proxyStreamSubs(Utils.fixLink(link, url), 'fancdn')
                };
            });
            return subtitles.length ? subtitles : false;
        }
    ...
    function fancdn2(component, _object) {
        ...
        var embed = 'https://fancdn.net/iframe/';
    ...
    function fanserials(component, _object) {
        ...
        function parseStream(element, call, error, url, str) {
            ...
            if (player) {
                ...
                if (json && json.hls) {
                    file = Utils.fixLink(json.hls, url);
                }
                ...
                ['data-original_subtitle', 'data-ru_subtitle', 'data-en_subtitle', 'data-ua_subtitle'].forEach(function (sub) {
                    ...
                    if (link) {
                        subtitles.push({
                            ...
                            url: component.processSubs(Utils.fixLink(link, url))
                        });
                    }
                });
            }
    ...
    function videoseed(component, _object) {
        ...
        var user_agent = Utils.baseUserAgent();
        var embed = 'https://tv-2-kinoserial.net/api_player.php';
        var suffix = 'token=0fbb7195cd5fe8f94d51bdabaed39461';
        var headers = Lampa.Platform.is('android') ? {
            'User-Agent': user_agent
        } : {};
        var prox_enc = '';
        ...
        this.search = function (_object, kinopoisk_id) {
            ...
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(api, prox, prox_enc, 'enc2'), function (json) {
    ...
    function vibix(component, _object) {
        ...
        var user_agent = Utils.baseUserAgent();
        var auth = 'Bearer 6087|DXDcUQ38W28kwGH4AE0oFjC4QcjVO6KTrqu7Our39c2e5b8e';
        var headers = Lampa.Platform.is('android') ? {
            'User-Agent': user_agent,
            'Authorization': auth
        } : {};
        ...
        var embed = 'https://vibix.org/api/v1/publisher/videos/';
        var filter_items = {};
        var choice = {
            season: 0,
            voice: 0,
        ...
        function vibix_api_search(api, callback, error) {
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(embed + api, prox, prox_enc), function (json) {
    ...
    function alloha(component, _object) {
        ...
        function alloha_api_search(api, callback, error) {
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(embed + '&' + api, prox, '', 'enc2'), function (json) {
                if (callback) callback(json);
            }, function (a, c) {
                if (error) error(network.errorDecode(a, c));
            });
        ...
        function getPage(data) {
            network.clear();
            network.timeout(20000);
            network["native"](Utils.proxyLink(data.iframe, prox2, prox2_enc, 'enc2'), function (str) {
        ...
        function parseSubs(tracks) {
            ...
            var subtitles = tracks.filter(function (t) {
                ...
                return {
                    label: item.label,
                    url: component.processSubs(Utils.proxyLink(link, prox2, extract.stream_prox2))
                };
            });
            return subtitles.length ? subtitles : false;
        }
        ...
        function getStream(element, call, error) {
            ...
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(extract.domain + 'api/movies/' + element.media.id, prox2, extract.prox2, 'enc2'), function (json) {
                        ...
                        for (var q_id in hlsSource.quality) {
                            ...
                            if (link) {
                                items.push({
                                    ...
                                    file: Utils.proxyLink(link, prox2, extract.stream_prox2)
                                });
                            }
                        }
    ...
    function redheadsound(component, _object, prefer_dash) {
        ...
        this.search = function (_object, kinopoisk_id, data) {
            ...
            var query_search = function query_search(query, data, callback) {
                ...
                network.clear();
                network.timeout(10000);
                network["native"](Utils.proxyLink(url, prox), function (str) {
                    str = (str || '').replace(/\n/g, '');
        ...
        function getPage(card) {
            ...
            var url = Utils.fixLink(card.link, ref);
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(url, prox), function (str) {
                str = (str || '').replace(/\n/g, '');
                ...
                if (player_link) {
                    network.clear();
                    network.timeout(10000);
                    network["native"](Utils.proxyLink(player_link, prox, prox_enc), function (str) {
                        parse(str);
                    }, function (a, c) {
                        component.empty(network.errorDecode(a, c));
                    }, false, {
    ...
    function cdnvideohub(component, _object) {
        ...
        var embed = 'https://plapi.cdnvideohub.com/api/v1/player/sv/';
        var filter_items = {};
        var choice = {
            season: 0,
            voice: 0,
        ...
        this.search = function (_object, kinopoisk_id) {
            ...
            var url = Lampa.Utils.addUrlComponent(embed + 'playlist?pub=12&aggr=kp', 'id=' + kinopoisk_id);
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(url, prox), function (json) {
                parse(json);
            }, function (a, c) {
        ...
        function extractItems(sources) {
            ...
            items.forEach(function (item) {
                item.file = Utils.fixLinkProtocol(item.file, prefer_http, true);
            });
            return items;
        }
    ...
    function anilibria(component, _object) {
        ...
        this.search = function (_object, kinopoisk_id, data) {
            ...
            network.silent(Utils.proxyLink(url, prox), function (json) {
    ...
    function animelib(component, _object) {
        ...
        function getEpisodes(json) {
            ...
            network.clear();
            network.timeout(1000 * 15);
            network.silent(Utils.proxyLink(url, prox, prox_enc), function (episodes) {
                ...
                if (player && player.video && player.video.quality) {
                    ...
                    items = player.video.quality.map(function (q) {
                        return {
                            ...
                            quality: q.quality,
                            file: q.href ? Utils.proxyLink(server.url + q.href, prox, prox_enc) : ''
                        };
                    });
                    items.sort(function (a, b) {
                        if (b.quality > a.quality) return 1;
    ...
    function kodik(component, _object) {
        ...
        var token = '45c53578f11ecfb74e31267b634cc6a8';
        var last_player = '';
        var last_info = '';
        var filter_items = {};
        var choice = {
        ...
        function kodik_api_search(api, callback, error) {
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(embed + api, prox), function (json) {
                if (callback) callback(json);
            }, function (a, c) {
                if (error) error(network.errorDecode(a, c));
            });
        ...
        function getStream(element, call, error) {
            ...
            var url = Utils.fixLinkProtocol(element.link, prefer_http);
            network.clear();
            network.timeout(10000);
            network["native"](Utils.proxyLink(url, prox), function (str) {
                str = (str || '').replace(/\n/g, '');
                ...
                if (postdata && player) {
                    var getLinks = function getLinks() {
                        network.clear();
                        network.timeout(10000);
                        network["native"](Utils.proxyLink(last_info, prox), function (json) {
                            if (json && json.links) {
                    ...
                    if (player_url !== last_player) {
                        network.clear();
                        network.timeout(10000);
                        network["native"](Utils.proxyLink(player_url, prox), function (str) {
                            str = (str || '').replace(/\n/g, '');
                ...
                if (file.url) {
                    ...
                    if (file.url.http && file.file) {
                        ...
                        if (base) {
                            ...
                            base_url = Utils.fixLinkProtocol(base_url, prefer_http, 'full');
                    ...
                    if (file.url.hls4) {
                        ...
                        if (_base) {
                            ...
                            _base_url = Utils.fixLinkProtocol(_base_url, prefer_http, 'full');
                            items.push({
                                height: file.h,
                                quality_id: file.quality_id,
                                label: label,
                    ...
                    if (file.url.hls2) {
                        ...
                        if (_base2) {
                            ...
                            _base_url2 = Utils.fixLinkProtocol(_base_url2, prefer_http, 'full');
                            items.push({
                                height: file.h,
                                quality_id: file.quality_id,
                                label: label,
                    ...
                    if (file.url.hls && file.file) {
                        ...
                        if (_base3) {
                            ...
                            _base_url3 = Utils.fixLinkProtocol(_base_url3, prefer_http, 'full');
                            items.push({
                                height: file.h,
                                quality_id: file.quality_id,
                                label: label,
    ...
    function component(object) {
        ...
        var all_sources = [{
            ...
            // kvk.zone, rezka.fi, rezka.ag
            // Proxy: iqslgbok.deploy.cx
            name: 'rezka2',
            title: 'HDrezka',
    ...
    Lampa.Settings.listener.follow('open', function (e) {
        if (e.name == 'filmix') {
            e.body.find('[data-name="filmix_add"]').unbind('hover:enter').on('hover:enter', function () {
                ...
                network.clear();
                network.timeout(10000);
                network["native"](Utils.proxyLink(api_url + 'token_request' + Utils.filmixToken(dev_id, ''), prox, prox_enc, 'enc2'), function (found) {
    ...
    function checkPro(token, call) {
        ...
        network["native"](Utils.proxyLink(api_url + 'user_profile' + Utils.filmixToken(dev_id, token), prox, prox_enc, 'enc2'), function (json) {
            if (json) {
                if (json.user_data) {
    ...
    function rezka2FillCookie(success, error) {
        ...
        if (proxy_mirror && !prox && !returnHeaders) {
            if (error) error();
            return;
        }
    ...
    template += "<div class=\"settings-param-title\"><span>Use proxy</span></div>";
    ...
    template += "<div class=\"settings-param-title\"><span>Proxy settings</span></div>";
    ...
    template += "<div class=\"settings-param-title\"><span>#{online_mod_balanser}</span></div>";
    ...
    template += "<div class=\"settings-param-title\"><span>Kinobase</span></div>";
    ...
    template += "<div class=\"settings-param-title\"><span>HDRezka</span></div>";
    ...
    // Куки для HDrezka, Заполнить куки для HDrezka
    ...
    template += "<div class=\"settings-param-title\"><span>FanSerials</span></div>";
    ...
    template += "<div class=\"settings-param-title\"><span>FanCDN</span></div>";
